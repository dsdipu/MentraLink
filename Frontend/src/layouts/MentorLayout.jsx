...
const navItems = [
  ...
  { to: "/mentor/sessions", label: "Sessions", icon: CalendarClock },
  { to: "/mentor/attendance", label: "Attendance", icon: ClipboardCheck },
...
const MentorLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-emerald-50">
      ...
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-mentor-gradient-vertical text-white flex flex-col z-40 transform transition-transform duration-200 ease-in-out ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0`}
      >
        <div className="py-4 bg-white border-b border-white/20 flex items-center justify-between px-4">
          <Link to="/">
            <img src={logo} alt="MentraLink" className="h-14 object-contain" />
          </Link>
          <button
